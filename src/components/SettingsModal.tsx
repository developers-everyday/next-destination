...
export default function SettingsModal({ }: SettingsModalProps) {
    ...
    const handleSave = () => {
        ...
        // For Mapbox, a reload might be safer to ensure the new token is picked up cleanly if the map is already mounted.
        if (localMapboxToken !== mapboxToken) {
            if (confirm("Changing the Mapbox token might require a page reload to take full effect. Reload now?")) {
    ...
    const handleReset = () => {
        if (confirm("Are you sure you want to reset settings to default environment variables?")) {
            resetToDefaults();
            // We'll update local state in the useEffect when the store updates
        }
    };

    if (!isSettingsOpen) {
        // console.log("SettingsModal: isSettingsOpen is false");
        return null;
    }

    console.log("SettingsModal: Rendering with isSettingsOpen true");
